...
ctd_temp = ctd %>%
  filter(!is.na(Temperature)) %>%
  filter(QF_Temperature == 2)
```

```{r}
min(ctd_temp$Temperature)
max(ctd_temp$Temperature)
mean(ctd_temp$Temperature)
```
...
get_mode(ctd_temp$Temperature)

```

```{r}
hist(ctd_temp$Temperature, xlim = c(0, 35))
```
